...
def create_filtered_test_case(test_path):
    ...
    if len(parts) < 2:
        raise ValueError("Invalid test path format. Use 'module.TestClass' or 'module.TestClass.test_method'")
    ...
    test_methods = [name for name in dir(original_test_class) 
    ...
    if target_method_name not in test_methods:
        ...
        raise ValueError(f"Test method '{target_method_name}' not found in {class_name}. Available test methods: {available_methods}")
